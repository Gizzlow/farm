...
public class TreatmentService {
	...
	public ResponseDTO<Object> addTreatment(TreatmentDTO treatmentDTO) {
		InventoryMedicine medicineEntity = inventoryMedicineRepository.getInventoryMedicineById(treatmentDTO.getInventoryId());
		...
		if(treatmentDTO.getAmount() <= medicineEntity.getAmount()) {
			...
			treatment.setMedicineType(treatmentDTO.getMedicineType());
			treatment.setRetireTime(treatmentDTO.getRetireTime());
			...
			responseDTO = ResponseDTO.builder().statusCode(ConstantTreatment.REMOVE_INVENTORY_MEDICINE_AMOUNT_SUCCESS.getStatusCode())
					.message(ConstantTreatment.REMOVE_INVENTORY_MEDICINE_AMOUNT_SUCCESS.getMessage()).object(treatmentDTO).build();
		}else {
			responseDTO = ResponseDTO.builder().statusCode(ConstantTreatment.REMOVE_INVENTORY_MEDICINE_AMOUNT_FAIL.getStatusCode())
					.message(ConstantTreatment.REMOVE_INVENTORY_MEDICINE_AMOUNT_FAIL.getMessage()).object(treatmentDTO).build();
		}
		return responseDTO;
	}
